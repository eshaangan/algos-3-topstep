"""Paper-trade monday_rth_v1: long MNQ Monday 9:30->16:00 ET. Stream quotes, no orders."""
import glob, json, os, time
from datetime import datetime, timezone
import pandas as pd
NC, PV, COMM, TICK = 2, 2.0, 0.62, 0.25
ET = "America/New_York"
def log(m): print(f"[{datetime.now(timezone.utc):%m-%d %H:%M:%S}Z] {m}", flush=True)
def quote(raw):
    fs = sorted(glob.glob(os.path.join(raw, "stream_MNQ_*.csv")))
    if not fs: return None, 1e9
    with open(fs[-1], "rb") as f:
        try: f.seek(-300, 2)
        except OSError: pass
        for line in reversed(f.read().decode(errors="ignore").strip().splitlines()):
            p = line.split(",")
            if len(p) == 5:
                try: return (float(p[1])+float(p[3]))/2, time.time()-float(p[0])/1e9
                except ValueError: continue
    return None, 1e9
def main():
    import argparse
    ap = argparse.ArgumentParser(); ap.add_argument("--raw-dir", required=True); ap.add_argument("--out-dir", required=True)
    a = ap.parse_args(); os.makedirs(a.out_dir, exist_ok=True)
    sp = os.path.join(a.out_dir, "monday_state.json"); tp = os.path.join(a.out_dir, "monday_trades.jsonl")
    st = json.load(open(sp)) if os.path.exists(sp) else {"open": None, "done": []}
    log("monday paper runner up")
    while True:
        now = pd.Timestamp.now(tz=ET); key = str(now.date())
        if now.weekday() == 0 and key not in st["done"]:
            ent = now.normalize() + pd.Timedelta(hours=9, minutes=30)
            ext = now.normalize() + pd.Timedelta(hours=15, minutes=59)
            if st["open"] is None and ent <= now <= ent + pd.Timedelta(minutes=10):
                mid, age = quote(a.raw_dir)
                if mid and age < 180:
                    st["open"] = {"day": key, "entry": mid}
                    open(tp, "a").write(json.dumps({"ev": "entry", "day": key, "px": mid}) + "\n")
                    log(f"PAPER ENTRY long 2 MNQ @ {mid:.2f}")
            if st["open"] and st["open"]["day"] == key and now >= ext:
                mid, age = quote(a.raw_dir)
                if mid:
                    pnl = (mid - st["open"]["entry"])*PV*NC - 2*COMM*NC - 2*TICK*PV*NC
                    open(tp, "a").write(json.dumps({"ev": "exit", "day": key, "px": mid, "pnl": round(pnl, 2)}) + "\n")
                    log(f"PAPER EXIT @ {mid:.2f} pnl ${pnl:+.2f}")
                    st["done"].append(key); st["open"] = None
        json.dump(st, open(sp, "w")); time.sleep(60)
if __name__ == "__main__": main()
