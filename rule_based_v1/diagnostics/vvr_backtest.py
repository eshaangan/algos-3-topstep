"""
VVR (Void Velocity Reversion) Backtest
Intraday 1-minute MES futures — fades downside liquidity-vacuum moves.
Data: Jun–Dec 2025, RTH only (09:30–15:59 ET).
"""
...
DATA_PATH = "data/processed/mes_1m_bars_cache.h5"
RESULTS_PATH = "rule_based_v1/diagnostics/vvr_results.json"
...
def load_rth_bars() -> pd.DataFrame:
    df = pd.read_hdf(DATA_PATH, key="/bars_1m")
    df["timestamp"] = pd.to_datetime(df["timestamp"], utc=True)
    # Set as tz-aware index BEFORE filtering so hour/minute use ET
    df = df.set_index("timestamp")
    df.index = df.index.tz_convert("US/Eastern")
    df = df.sort_index()

    # RTH filter: 09:30–15:59 ET (minute 570–959)
    minute_of_day = df.index.hour * 60 + df.index.minute
    df = df[(minute_of_day >= 570) & (minute_of_day <= 959)].copy()

    # Bring index back as column; keep tz-aware Series for time extraction
    df = df.reset_index()  # 'timestamp' column is now tz-aware US/Eastern
    return df
...
def compute_rolling_metrics(df: pd.DataFrame):
    ...
    for i in range(n):
        ...
        atr_1m[i] = np.mean(atr_buf)
        med_range[i] = np.median(shock_buf)

    return atr_1m, med_range
...
def run_backtest(
    ...
    # Use the tz-aware timestamp Series for ET time extraction
    ts_series = df["timestamp"]  # tz-aware US/Eastern Series
    ...
    atr_1m, med_range = compute_rolling_metrics(df)

    # Extract ET hour/minute per bar using the tz-aware Series
    bar_hour = ts_series.dt.hour.values
    bar_minute = ts_series.dt.minute.values
    bar_date = ts_series.dt.date.values  # numpy array of date objects

    eps = 1e-9
    trades = []

    # Rolling history deques for percentile computation
    shock_history = deque(maxlen=SHOCK_WINDOW)
    deg_history = deque(maxlen=DEG_WINDOW)
    orx_history = deque(maxlen=ORX_WINDOW)

    # Session state (reset per calendar day)
    current_date = None
    consec_losses = 0
    session_losses = 0
    pause_until_bar = -1
    ...
    # Active gap watches: list of dicts (max MAX_ACTIVE_GAPS)
    active_gaps = []
    ...
    while i < n:
        ...
        if i >= 2 and atr_1m[i] > 0 and med_range[i] > 0:
        ...
        for gap in active_gaps:
            ...
            # Expire gaps from a previous session day
            ...
            if gap["state"] == "waiting_revisit":
                if i > gap_bar_idx and high[i] >= void_lo:
                    ...
                    if valid_revisit:
                        ...
                        # First revisit invalid (closed in bottom quartile or
                        # fully traversed void) — discard gap
                        continue
                else:
                    new_active_gaps.append(gap)
                ...
                if i == revisit_idx + 1:
                    if high[i] >= revisit_high:
                        ...
                        if can_enter:
                            ...
                            # tp1 = gap midpoint; only enter if geometry valid
                            profit_target = gap_mid
                            if profit_target <= entry_price:
                                # gap_mid below entry — use tp2 (full fill)
...
def compute_stats(trades: list, n_rth_bars: int, n_days: int) -> dict:
    ...
    if pnls.std() > 0:
        sharpe_trade = float((pnls.mean() / pnls.std()) * math.sqrt(252 * (N / n_days)))
    else:
        sharpe_trade = 0.0
    ...
    if len(daily_arr) > 1 and daily_arr.std() > 0:
        sharpe_daily = float((daily_arr.mean() / daily_arr.std()) * math.sqrt(252))
    else:
        sharpe_daily = 0.0

    avg_trades_per_day = float(N / n_days)
...
def monotonicity_test(trades: list) -> dict:
    if len(trades) < 10:
        return {"monotonicity_shock": [], "monotonicity_deg": [], "monotonicity_orx": []}
    ...
    def decile_breakdown(field: str, invert: bool = False) -> list:
        ...
        for d in range(10):
            ...
            subset = pnls[mask]
            if len(subset) == 0:
                result.append({"decile": f"{d*10}-{(d+1)*10}", "N": 0,
                               "WR": None, "avg_pnl": None})
            else:
                result.append({
...
def threshold_sweep(df: pd.DataFrame, n_days: int) -> dict:
    shock_results = []
    for sp in range(60, 100, 10):
        t = run_backtest(df, shock_pctl=sp, deg_pctl=DEG_PCTL, collect_trades=True)
        if not t:
            shock_results.append({"shock_pctl": sp, "N": 0,
        ...
        daily_arr = np.array(list(daily_pnl.values()))
        sharpe = (
            round(float((daily_arr.mean() / daily_arr.std()) * math.sqrt(252)), 3)
            if len(daily_arr) > 1 and daily_arr.std() > 0
            else 0.0
        )
    ...
    for dp in range(30, 80, 10):
        t = run_backtest(df, shock_pctl=SHOCK_PCTL, deg_pctl=dp, collect_trades=True)
        if not t:
            deg_results.append({"deg_pctl": dp, "N": 0,
                                 "WR": None, "avg_pnl": None, "sharpe": None})
            continue
        pnls = np.array([x["pnl"] for x in t])
        daily_pnl = {}
    ...
    return {"threshold_sweep_shock": shock_results, "threshold_sweep_deg": deg_results}
...
def print_summary(results: dict):
    ...
    ov = results["overall"]
    print("\n" + "=" * 62)
    print("  VVR (Void Velocity Reversion) Backtest — MES 1-min")
    print("=" * 62)
    print(f"  Data          : {meta['data_range']}")
    print(f"  Days / Bars   : {meta['n_days']} RTH days, {meta['n_rth_bars']:,} bars")
    print(f"  Trades        : {ov['N']}")
    if ov["N"] == 0:
    ...
    for row in results["exit_breakdown"]:
        ...
        avg_str = f"${row['avg_pnl']:.2f}" if row["avg_pnl"] is not None else "  N/A"
    ...
    for label, key in [
        ...
        for row in rows:
            wr_str = f"{row['WR']*100:.1f}%" if row["WR"] is not None else "  N/A"
            avg_str = f"${row['avg_pnl']:.2f}" if row["avg_pnl"] is not None else "  N/A"
            print(f"  {row['decile']:<10} {row['N']:>5} {wr_str:>7} {avg_str:>9}")
        print()

    print("  Threshold Sweep — SHOCK_PCTL:")
    print(f"  {'SHOCK_PCTL':<12} {'N':>5} {'WR':>7} {'AvgPnL':>9} {'Sharpe':>8}")
    print("  " + "-" * 45)
    for row in results["threshold_sweep_shock"]:
        wr_str = f"{row['WR']*100:.1f}%" if row["WR"] is not None else "  N/A"
        avg_str = f"${row['avg_pnl']:.2f}" if row["avg_pnl"] is not None else "  N/A"
        sh_str = f"{row['sharpe']:.3f}" if row["sharpe"] is not None else "  N/A"
        print(f"  {row['shock_pctl']:<12} {row['N']:>5} {wr_str:>7} {avg_str:>9} {sh_str:>8}")
    print()

    print("  Threshold Sweep — DEG_PCTL:")
    print(f"  {'DEG_PCTL':<12} {'N':>5} {'WR':>7} {'AvgPnL':>9} {'Sharpe':>8}")
    print("  " + "-" * 45)
    for row in results["threshold_sweep_deg"]:
        wr_str = f"{row['WR']*100:.1f}%" if row["WR"] is not None else "  N/A"
        avg_str = f"${row['avg_pnl']:.2f}" if row["avg_pnl"] is not None else "  N/A"
        sh_str = f"{row['sharpe']:.3f}" if row["sharpe"] is not None else "  N/A"
        print(f"  {row['deg_pctl']:<12} {row['N']:>5} {wr_str:>7} {avg_str:>9} {sh_str:>8}")
    print("=" * 62)
...
def main():
    ...
    print(f"  Bar time range: {ts_s.iloc[0]} to {ts_s.iloc[-1]}")

    print("Running base backtest...")
    trades = run_backtest(df, shock_pctl=SHOCK_PCTL, deg_pctl=DEG_PCTL, collect_trades=True)
    ...
    print("Running threshold sweeps...")
    sweeps = threshold_sweep(df, n_days)
