...
class DailyTopNSelector:
    ...
    def should_enter(
        ...
        *,
        ...
        if score is None or (isinstance(score, float) and np.isnan(score)):
            self.last_rejection_reason = "no_score"
            return False

        if self.trades_today >= self.max_trades_per_day:
            self.last_rejection_reason = "daily_budget"
            return False
        ...
        if score < floor:
            self.last_rejection_reason = "score_below_threshold"
            return False

        if bar_index is not None and not self._is_in_top_n(int(bar_index), direction):
            self.last_rejection_reason = "ranked_out"
            return False
    ...
    def log_rejection(self, prefix: str = "Signal rejected") -> None:
        if self.last_rejection_reason and LOGGER.isEnabledFor(logging.DEBUG):
            LOGGER.debug("%s: reason=%s", prefix, self.last_rejection_reason)
