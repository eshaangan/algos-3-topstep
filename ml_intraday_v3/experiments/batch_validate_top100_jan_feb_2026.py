#!/usr/bin/env python3
"""
Batch Validation of Top 100 Models on Jan-Feb 2026 OOS Data

Tests top 100 models from exhaustive search on the 41-day out-of-sample
period (Jan 1 - Feb 10, 2026) to find which configs actually work.
...
def main():
    logger.info("="*80)
    logger.info("BATCH VALIDATION: TOP 100 MODELS ON JAN-FEB 2026")
    logger.info("="*80)
    ...
    # Load top 100 configs
    cache_dir = Path("/tmp/phasefull_results")

    # Load all result JSONs
    logger.info("\nLoading experiment results...")
    result_files = sorted(cache_dir.glob("result_exhaustive_exp_*.json"))

    configs = []
    for result_file in result_files[:100]:  # Top 100
        with open(result_file) as f:
            config = json.load(f)
            configs.append(config)

    logger.info(f"  Loaded {len(configs)} model configs")
    ...
    output_path = ml_root / "diagnostics" / f"batch_validation_top100_jan_feb_2026_{timestamp}.csv"
