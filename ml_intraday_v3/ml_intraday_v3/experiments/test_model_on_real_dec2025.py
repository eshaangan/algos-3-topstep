#!/usr/bin/env python3
"""
Test Trained Model on REAL December 2025 MES Data

This script:
1. Loads REAL Dec 2025 MES data (5-minute bars from existing backtest)
...
9. Compares to January 2026 results

This will validate the model on the FIRST OUT-OF-SAMPLE MONTH after
training ended (Nov 2025). Expected to show BETTER performance than
Jan 2026 (no regime shift).
"""
...
def print_results(trades_df, daily_df):
    ...
    print("\n" + "="*80)
    print("REAL DEC 2025 MODEL PERFORMANCE RESULTS")
    print("="*80)
...
def main():
    print("="*80)
    print("TESTING TRAINED MODEL ON REAL DECEMBER 2025 MES DATA")
    print("="*80)
    print()

    # Step 1: Load real Dec 2025 data
    print("STEP 1: LOAD REAL DECEMBER 2025 DATA")
    print("-"*80)

    data_file = Path("backtest_results/dec_2025_test/bar_size=5m/bars.parquet")
    ...
    print(f"   Trading days: {len(pd.unique(bars_df.index.date))}")
    print()
    ...
    trades_file = output_dir / "dec2025_real_trades.csv"
    daily_file = output_dir / "dec2025_real_daily.csv"
